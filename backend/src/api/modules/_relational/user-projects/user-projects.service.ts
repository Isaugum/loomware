...
export class UserProjectsService {
    constructor(
        ...
        @InjectRepository(UserProject) private readonly userProjectsRepository: Repository<UserProject>,
    ) {}

    async bindProject({ projectId, userId }: CreateUserProjectDto): Promise<{ success: boolean, message: string }>
    {
        if(!projectId || !userId) {
        ...
        try {
            const newProject = this.userProjectsRepository.create({ projectId, userId });
            ...
            this.logger.error('Something went wrong while creating the project: ' + err);
            throw new InternalServerErrorException('Something went wrong while creating the project.');
        }
    }
    ...
    async getUserProjects(userId: User['id']): Promise<Project[]>
    {
        ...
        try {
            ...
            throw new InternalServerErrorException('Something went wrong while fetching project.');
        }
    }
}
